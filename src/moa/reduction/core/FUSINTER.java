...
public class FUSINTER {
	...
	*/
	 ...
	 */	
		 ...
		 * @param _end
		 */
		public Interval(int []_values,int _begin,int _end) {
	...
	protected Vector discretizeAttribute(int attribute,int []values,int begin,int end) {
		...
		while(intervals.size()>1 && !exit) {
			int posMin=-1;
			double maxCri=0;
			double eval = eval_2 (intervals);
			for(int i=0;i<intervals.size()-1;i++) {
				criterion = eval - eval_2(intervals, i);
				if(posMin==-1) {
					posMin = i;
					maxCri = criterion;
				} else {
	...
	double eval_2 (Vector <Interval> intervals) {
	...
	double eval_2 (Vector <Interval> intervals, int merged) {
